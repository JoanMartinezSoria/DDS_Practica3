...
datos_pie <- datos_pie %>%
  # Convertimos los nombres de fila (las categorías) en una columna real
  mutate(Categoria = rownames(.)) %>%
  # ¡IMPORTANTE!: Eliminamos la fila que dice "Total" para no duplicar datos
  filter(Categoria != "Total") %>%
  # Opcional: Si quieres quitar los NAs (summarytools suele incluir <NA>)
  # filter(!is.na(Categoria) & Categoria != "<NA>") %>% 
  mutate(
    # summarytools usa la columna 'Freq' para el conteo
    Porcentaje = Freq / sum(Freq) * 100,
    # Creamos la etiqueta de texto
    Etiqueta = paste0(round(Porcentaje, 1), "%"),
    # Calculamos la posición para que el texto quede centrado en cada sector
    y_pos = cumsum(Porcentaje) - 0.5 * Porcentaje
  )

# 3. CREAR EL GRÁFICO (PIE CHART)
ggplot(datos_pie, aes(x = "", y = Porcentaje, fill = Categoria)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y", start = 0) +
  geom_text(aes(y = y_pos, label = Etiqueta), color = "white", fontface = "bold") +
  theme_void() +
